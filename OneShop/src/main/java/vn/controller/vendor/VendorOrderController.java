...
public class VendorOrderController {
    ...
    public String confirmOrder(@RequestParam("orderId") Long orderId,
        ...
        try {
            ...
            // ===== Mail HTML đẹp do OrderStatusEmailComposer + Observer gửi =====
    ...
    public String orderDetail(@PathVariable Long orderId, HttpSession session, Model model) {
        ...
        String orderStatusName = order.getStatus() != null ? order.getStatus().name() : "PENDING";
        boolean hasRefund = refund != null;
        
        // Determine refund status name with proper fallback logic
        String refundStatusName;
        if (refund != null && refund.getRefundStatus() != null) {
            refundStatusName = refund.getRefundStatus().name();
            
            // Special case: If refund status is PENDING but has bank info, treat as PROCESSING
            // This handles the case where user submitted bank info but status wasn't updated correctly
            if (refundStatusName.equals("PENDING") && 
                refund.getRefundMethod() != null && 
                refund.getRefundMethod() == vn.entity.Refund.RefundMethod.BANK_TRANSFER &&
        ...
        model.addAttribute("hasRefund", hasRefund);
        model.addAttribute("refundStatusName", refundStatusName);
        model.addAttribute("refundMethodName", refundMethodName);
    ...
    public String approveReturn(@PathVariable Long orderId,
        ...
        try {
            ...
            // Chỉ có thể duyệt hoàn khi đơn hàng ở trạng thái RETURN_REQUESTED (đã yêu cầu trả hàng)
            if (order.getStatus() != Order.OrderStatus.RETURN_REQUESTED) {
                redirectAttributes.addFlashAttribute("error", "Chỉ có thể duyệt hoàn khi khách hàng đã yêu cầu trả hàng");
                return "redirect:/vendor/orders/" + orderId;
            }
            ...
            if (refundAmount <= 0 || refundAmount > finalAmount) {
                redirectAttributes.addFlashAttribute("error", "Số tiền hoàn không hợp lệ. Số tiền tối đa có thể hoàn: " + 
                    String.format("%.0f", finalAmount) + " VNĐ");
                return "redirect:/vendor/orders/" + orderId;
            }
            ...
            if (returnedOrder != null) {
                String originalReason = order.getCancellationReason();
                returnedOrder.setCancellationReason("Return approved: " + originalReason + ". Vendor note: " + returnReason + ". Refund amount: " + refundAmount + " VND");
