...
public class FlashSaleServiceImpl implements FlashSaleService {
    ...
    public FlashSale updateFlashSale(Long id, FlashSale flashSale) {
        ...
        // Allow update of active flash sale only if not changing critical fields during active period
        // For ended flash sales, allow full update to extend time or reactivate
        if (existing.getStatus() == FlashSale.FlashSaleStatus.ACTIVE) {
            // Check if trying to change time while active
            LocalDateTime now = LocalDateTime.now();
            if (flashSale.getStartTime() != null && !flashSale.getStartTime().equals(existing.getStartTime())) {
                // If current time is after the new start time, allow update
                if (now.isAfter(flashSale.getStartTime())) {
                    // Allow update but warn
                    System.out.println("Warning: Updating start time of active flash sale " + id);
                } else {
                    throw new RuntimeException("Cannot change start time of active flash sale to future time");
                }
            }
        }
        
        // Allow update of ended flash sales - this enables extending time or reactivating
        // No restriction on updating ended flash sales
        ...
        // Auto-update status based on new time if flash sale was ended
        if (existing.getStatus() == FlashSale.FlashSaleStatus.ENDED || 
            existing.getStatus() == FlashSale.FlashSaleStatus.CANCELLED) {
            LocalDateTime now = LocalDateTime.now();
            
            // If new end time is in the future, check if we should reactivate
            if (existing.getEndTime().isAfter(now)) {
                // If start time has passed, activate immediately
                if (existing.getStartTime().isBefore(now) || existing.getStartTime().isEqual(now)) {
                    existing.setStatus(FlashSale.FlashSaleStatus.ACTIVE);
                    System.out.println("Auto-activated flash sale " + id + " after time extension");
                } else {
                    // Start time is in the future, schedule it
                    existing.setStatus(FlashSale.FlashSaleStatus.SCHEDULED);
                    System.out.println("Auto-scheduled flash sale " + id + " after time update");
                }
            }
            // If end time is still in the past, keep it as ENDED
        }
    ...
    public List<FlashSale> getActiveFlashSales() {
        LocalDateTime now = LocalDateTime.now();
        System.out.println("=== getActiveFlashSales ===");
        System.out.println("Current time: " + now);
        
        List<FlashSale> activeSales = flashSaleRepository.findActiveFlashSales(now);
        System.out.println("Found " + activeSales.size() + " active flash sales");
        
        // Also check all flash sales with ACTIVE status
        List<FlashSale> allActive = flashSaleRepository.findByStatus(FlashSale.FlashSaleStatus.ACTIVE);
        System.out.println("Total flash sales with ACTIVE status: " + allActive.size());
        allActive.forEach(fs -> {
            System.out.println("  - ID: " + fs.getFlashSaleId() + 
                             ", Name: " + fs.getSaleName() +
                             ", Start: " + fs.getStartTime() +
                             ", End: " + fs.getEndTime() +
                             ", Now: " + now +
                             ", Is in time range: " + (fs.getStartTime().isBefore(now) || fs.getStartTime().isEqual(now)) + 
                             " && " + (fs.getEndTime().isAfter(now)));
        });
    ...
    public Optional<FlashSaleProduct> getActiveFlashSaleProduct(Long productId) {
        LocalDateTime now = LocalDateTime.now();
        System.out.println("=== getActiveFlashSaleProduct ===");
        System.out.println("Product ID: " + productId + ", Current time: " + now);
        
        Optional<FlashSaleProduct> fspOpt = flashSaleProductRepository.findByProductIdAndActive(productId, now);
        
        if (fspOpt.isPresent()) {
            FlashSaleProduct fsp = fspOpt.get();
            System.out.println("Found flash sale product: " + fsp.getProduct().getProductName());
            System.out.println("Flash Sale: " + fsp.getFlashSale().getSaleName() + 
                             ", Status: " + fsp.getFlashSale().getStatus());
        } else {
            System.out.println("No active flash sale product found for product ID: " + productId);
        }
