...
public interface FlashSaleRepository extends JpaRepository<FlashSale, Long> {
    ...
    @Query("SELECT fs FROM FlashSale fs WHERE fs.status = vn.entity.FlashSale.FlashSaleStatus.ACTIVE " +
    ...
    @Query("SELECT fs FROM FlashSale fs WHERE fs.status = vn.entity.FlashSale.FlashSaleStatus.SCHEDULED " +
    ...
    @Query("SELECT DISTINCT fs FROM FlashSale fs JOIN fs.products fsp " +
           ...
           "AND fs.status = vn.entity.FlashSale.FlashSaleStatus.ACTIVE " +
           "AND fs.startTime <= :now AND fs.endTime > :now")
    ...
    @Query("SELECT COUNT(fs) FROM FlashSale fs WHERE fs.status = vn.entity.FlashSale.FlashSaleStatus.ACTIVE " +
           "AND fs.startTime <= :now AND fs.endTime > :now")
